<template>
  <div class="mt-5">
    <p class="text-amber-300 text-5xl font-bold hover:underline">Work.</p>
    <p class="mt-20">
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Autem aut fugiat
      iusto, impedit nam illo enim excepturi sit eaque accusamus quasi
      reiciendis error vero sed maxime odit quia corporis eveniet molestiae quae
      temporibus suscipit distinctio consequuntur repudiandae. Ea, aspernatur
      labore alias quam molestiae beatae ducimus deserunt ipsam, rem et ratione.
    </p>
    <div class="flex mt-10">
      <div class="">
        <img src="./../assets/com.jpg" alt="work image 1" />
        <p class="italic">Novenber 24, 2022</p>
        <p class="font-bold text-2xl mt-2">Some Case Study</p>
        <p>
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quibusdam
          molestias adipisci repellendus consequatur perferendis voluptates
          molestiae quod repudiandae autem amet, laudantium tempora minima eos
          veniam temporibus quas esse qui nobis ea ducimus iusto voluptas
          facilis earum. Quibusdam soluta at temporibus.
        </p>
      </div>
      <div class="">
        <img src="./../assets/comp2.jpg" alt="work image 1" />
        <p class="italic">May 31, 2023</p>
        <p class="font-bold text-2xl mt-2">Some Case Study</p>
        <p>
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quibusdam
          molestias adipisci repellendus consequatur perferendis voluptates
          molestiae quod repudiandae autem amet, laudantium tempora minima eos
          veniam temporibus quas esse qui nobis ea ducimus iusto voluptas
          facilis earum. Quibusdam soluta at temporibus.
        </p>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "WorkComponent",
};
</script>

<style></style>
